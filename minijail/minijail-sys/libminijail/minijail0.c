...
int main(int argc, char *argv[])
{
	...
	/*
	 * There are two minijail processes; this one (the parent) and
	 * one that becomes init(). By making this process the session
	 * group (and process group) leader, it becomes possible to kill -9
	 * all processes in the hierarchy. If any child process attempts
	 * to modify the terminal via TIOCSTI, it will not affect our parent,
	 * only this process that is waiting to reap signals in
	 * minijail_wait().
	 */
#ifdef ANDROID
	if (setsid() == (pid_t)-1) {
		int error = errno;
		fprintf(stderr,
			"Can not set session ID, error %d. "
		        "Group KILL not possible\n", error);
	}
#endif
	/*
