...
/* Network namespace workarounds */
#define IFDEVNAME_MAX 16
#define IPV4_BUFLEN 32
#define FLOCK_SLEEP_NS (1000 * 1000 * 10) /* 10 ms */
#define FLOCK_SLEEP_RETRY (100 * 5)       /* 5 secs */
#define IPADDR_LIST "/data/iotedge/ipaddr_hack"
#define IPADDR_TMP "/data/iotedge/ipaddr_tmp"
#define IPADDR_LOCKFILE "/data/iotedge/ipaddr_lock"
#define LOGWRAP "/system/bin/logwrapper"
#define NETNS_CMD "/system/bin/netns_veth.sh"
#define IPCMD_NSPATH "/var/run/netns"
...
static int lock_ipaddr_hack(void)
{
    int sleepcnt = 0, fd, retval = -1, error;
    struct timespec tm, rem;

    fd = open(IPADDR_LOCKFILE, O_RDONLY);
    if (fd < 0)
    {
        error = errno;
        warn("Can not open lockfile %s error %d\n",
             IPADDR_LOCKFILE, error);
        goto out;
    }
    ...
    do
    {
        ...
        tm.tv_sec = 0;
        tm.tv_nsec = FLOCK_SLEEP_NS;
        (void)nanosleep(&tm, &rem);
        sleepcnt++;
    } while (sleepcnt < FLOCK_SLEEP_RETRY);

    if (sleepcnt > FLOCK_SLEEP_RETRY)
    {
        (void)close(fd);

        warn("Could not get lock on ipaddr file");
        goto out;
    }
    retval = fd;
...
static void unlock_ipaddr_hack(int lockfd)
{
    int error;

    if (lockfd > 0)
    {
        error = flock(lockfd, LOCK_UN);
        if (error)
            warn("Can not unlock ipaddr file");
    }
}

/*
 * This is a horrid, horrid hack to get the demo working. We maintain
 * a list of all possible IP addresses in our subnet in a file
 * and remove one. When the process exits, we add it back
 */
static void get_ipaddr_hack(char *retbuf)
{
    char ipaddr[IPV4_BUFLEN], buf[IPV4_BUFLEN];
    FILE *fp = NULL, *tmp = NULL;
    int retval, error = 0, a, b, c, d, lockfd = -1;

    lockfd = lock_ipaddr_hack();
    if (lockfd < 0)
        die("Could not get lock on ipaddr hack file");

    fp = fopen(IPADDR_LIST, "r");
    if (fp == NULL)
        die("Could not open ipaddr hack file %s", IPADDR_LIST);

    /*
     * Make sure the file is in the correct IPv4 format
     */
    retval = fscanf(fp, "%d.%d.%d.%d", &a, &b, &c, &d);
    if (retval != 4)
    {
        error = EINVAL;
        warn("Bad input file, received %d octets", retval);
        goto out;
    }

    retval = snprintf(ipaddr, sizeof(ipaddr), "%d.%d.%d.%d", a, b, c, d);
    if (retval <= 0 || retval >= (int)sizeof(ipaddr))
    {
        error = EINVAL;
        warn("Can not convert IPv4 address");
        goto out;
    }
    debug("found ipv4 address %s", ipaddr);
    strcpy(retbuf, ipaddr);

    /*
     * Now that we have our address, rewrite the rest of the file
     */
    tmp = fopen(IPADDR_TMP, "w");
    if (tmp == NULL)
    {
        error = ENOENT;
        warn("Could not open tmpfile %s", IPADDR_TMP);
        goto out;
    }

    while (fscanf(fp, "%d.%d.%d.%d", &a, &b, &c, &d) != EOF)
    {
        retval = snprintf(buf, sizeof(buf), "%d.%d.%d.%d\n", a, b, c, d);
        if (retval <= 0 || retval >= (int)sizeof(buf))
        {
            error = EINVAL;
            warn("Can not convert IPv4 address on rewrite");
            goto out;
        }
        retval = fwrite(buf, strlen(buf), 1, tmp);
        if (retval == 0)
        {
            error = EIO;
            warn("Can not write tmp file\n");
            goto out;
        }
    }

    retval = rename(IPADDR_TMP, IPADDR_LIST);
    if (retval)
    {
        error = errno;
        warn("Can not rename tmpfile error %d", error);
        goto out;
    }
...
out:
    ...
    if (lockfd > 0)
        unlock_ipaddr_hack(lockfd);

    if (tmp)
        fclose(tmp);

    if (error)
        die("Can not get ipaddress, error %d", error);

    return;
}

static int put_ipaddr_hack(const char *ipaddr)
{
    char buf[IPV4_BUFLEN];
    FILE *fp = NULL;
    int retval, error = 0, lockfd = -1;

    lockfd = lock_ipaddr_hack();
    if (lockfd < 0)
    {
        error = EACCES;
        warn("Could not get lock on ipaddr hack file");
        goto out;
    }

    /*
     * Write back to the end so we round-robin
     */
    fp = fopen(IPADDR_LIST, "a");
    if (fp == NULL)
    {
        error = EINVAL;
        warn("Could not open ipaddr hack file %s", IPADDR_LIST);
        goto out;
    }

    retval = snprintf(buf, sizeof(buf), "%s\n", ipaddr);
    if (retval <= 0 || retval >= (int)sizeof(buf))
    {
        error = EINVAL;
        warn("Can not convert IPv4 address on rewrite");
...
static int del_namespace_from_bridge(const char *ns_devname)
{
    int retval, error = 0;
    char cmdbuf[128];

    debug("start remove namespace %s", ns_devname);

    retval = snprintf(cmdbuf, sizeof(cmdbuf), "%s %s del %s",
                      LOGWRAP, NETNS_CMD, ns_devname);
    if (retval <= 0 || retval >= (int)sizeof(cmdbuf))
    {
        error = EINVAL;
        goto out;
    }
    ...
    debug("end remove namespace %s", ns_devname);
out:
    return error;
}

static int add_namespace_to_bridge(const char *ipaddr, const char *ns_devname,
                                   const char *br_name)
{
    int retval, error = 0;
    char cmdbuf[128];

    debug("start add namespace %s to bridge %s with %s",
          ns_devname, br_name, ipaddr);

    retval = snprintf(cmdbuf, sizeof(cmdbuf), "%s %s add %s %s %s",
                      LOGWRAP, NETNS_CMD, ns_devname, ipaddr, br_name);
    if (retval <= 0 || retval >= (int)sizeof(cmdbuf))
    {
        error = EINVAL;
        goto out;
    ...
    debug("end add namespace %s to bridge %s with %s",
          ns_devname, br_name, ipaddr);
out:
    return error;
}
...
/*
 * ESR workaround: create a namespace using the system scripts
 *
 * ASSUMPTIONS:
 * 	A file named /data/iotedge/ipaddr_hack exists will the
 * 		list of ipaddrs to be used
 * 	A file name /data/iotedge/ipaddr_lock exists
 * 	The bridge has already been initialized
 * 	There are setup scripts in /system/bin
 */
void API minijail_namespace_create_net(struct minijail *j,
                                       const char *ns_name,
                                       const char *br_name)
{
    char ns_devname[IFDEVNAME_MAX];
    char ipaddr[IPV4_BUFLEN];
    char ns_pathbuf[64];
    int retval, error = 0;

    j->net_bridge = strdup(br_name);
    if (j->net_bridge == NULL)
        pdie("Can not alloc memory for network bridge name\n");

    /*
     * The ns_name is based on the process ID from the 'north' service
     * and is 32 chars; we truncate because the system scripts will add
     * 'veth' and 'br-veth' to it. The max devname for network devices
     * is 15 chars (plus a null)
     */
    memset(ns_devname, 0, sizeof(ns_devname));
    memcpy(ns_devname, ns_name, IFDEVNAME_MAX - (strlen("br-veth") + 1));

    j->net_nsname = strdup(ns_devname);
    if (j->net_nsname == NULL)
        die("Can not allocate memory for net namespace");

    retval = snprintf(ns_pathbuf, sizeof(ns_pathbuf), "%s/%s",
                      IPCMD_NSPATH, ns_devname);
    if (retval <= 0 || retval >= (int)sizeof(ns_pathbuf))
        die("Can not format namespace runtime name");

    debug("start configure namespace %s (%s) with bridge %s",
          ns_name, ns_devname, j->net_bridge);
    /*
     * Get an unused address from our hack file that is resident
     * on the system. This is a workaround until a list
     * can be implemented in the 'north' command processor
     */
    memset(ipaddr, 0, sizeof(ipaddr));
    get_ipaddr_hack(ipaddr);

    j->net_ipaddr = strdup(ipaddr);
    if (j->net_ipaddr == NULL)
    {
        error = ENOMEM;
        warn("Can not allocate memory for ipaddr");
        goto out;
    }

    error = add_namespace_to_bridge(ipaddr, ns_devname, j->net_bridge);
    if (error)
    {
        warn("Can not create namespace, error %d", error);
        goto out;
    }

    minijail_namespace_enter_net(j, ns_pathbuf);

    debug("end configure namespace %s (%s) with bridge %s",
          ns_name, ns_devname, j->net_bridge);

out:
    if (error)
    {
        (void)put_ipaddr_hack(ipaddr);
        die("Can not create network namespace");
    }
}
...
static int remount_proc_readonly(const struct minijail *j)
{
    ...
    debug("remounting /proc readonly");
    ...
    if (umount2(kProcPath, MNT_DETACH))
    {
        ...
        info("unmount of %s failed error %d", kProcPath, error);
...
void write_ugid_maps_or_die(const struct minijail *j)
{
    debug("write_ugid_maps pid %d tid %ld parent %d", getpid(), syscall(SYS_gettid), getppid());
...
{
    debug("drop_ugid pid %d tid %ld parent %d", getpid(), syscall(SYS_gettid), getppid());
...
#if defined(__ANDROID__)
    /*
     * We can only enter the chroot after remounting /proc,
     * otherwise we get EINVAL from the kernel's do_mount()
                                 ...
                                 struct minijail_run_status *status_out)
{
    ...
    if (pid_namespace)
    {
        ...
        debug("clone pid %d child_pid %d", getpid(), child_pid);
    }
    else
    {
        child_pid = fork();
     ...
     *      -> execve()-ing process
     */
    debug("executing %s", config->filename);
...
int API minijail_wait(struct minijail *j)
{
    ...
    /*
     * If we created a network namespace, free it up
     */
    if (j->net_nsname)
    {
        if (del_namespace_from_bridge(j->net_nsname))
            warn("Can not free namespace %s", j->net_nsname);
        free(j->net_nsname);
        j->net_nsname = NULL;
    }
    if (j->net_ipaddr)
    {
        if (put_ipaddr_hack(j->net_ipaddr))
            warn("Can not return ipaddr %s", j->net_ipaddr);
        free(j->net_ipaddr);
        j->net_ipaddr = NULL;
    }
