...
static void add_binding(struct minijail *j, char *arg)
{
	...
	if (minijail_bind(j, src, dest, atoi(flags))) {
		fprintf(stderr, "minijail_bind of %s to %s failed.\n", src, dest);
		exit(1);
	}
}
...
static void add_namespace(struct minijail *j, char *arg)
{
	char *ns = tokenize(&arg, ",");
	char *br_name = tokenize(&arg, ",");
	if (!ns || ns[0] == '\0' || arg != NULL) {
		fprintf(stderr, "Bad namespace parameter: %s %s\n",
			ns, br_name);
		exit(1);
	}
	minijail_namespace_create_net(j, ns, br_name);
}
...
static void usage(const char *progn)
{
	...
	printf("Usage: %s [-dGhHiIKlLnNprRstUvyYz]\n"
	       ...
	       "  -E<name,br>:  Create a new network name space and link the veth devices to the specified bridge\n"
	       ...
	       const char **preload_path)
{
	...
	const char *optstring =
	    "+u:g:sS:c:C:P:b:B:V:f:m::M::k:a:E:e::R:T:vrGhHinNplLt::IUK::wyYzZ:d";
	/* clang-format off */
