...
impl Minijail {
    ...
    pub fn log_to_fd(&self, fd: RawFd, priority: LogPriority) {
        let priority = priority as i32;
        unsafe {
            minijail_log_to_fd(fd as i32, priority);
        }
    }
    ...
    pub fn run(
        ...
        // into a null terminated array, suitable for use as an env parameter to `execve`.
        let mut env_cstr = Vec::with_capacity(env.len());
        let mut env_array = Vec::with_capacity(env.len());
        for &env_var in env {
            let env_var_cstr =
                CString::new(env_var).map_err(|_| Error::StrToCString(env_var.to_owned()))?;
            env_array.push(env_var_cstr.as_ptr());
            env_cstr.push(env_var_cstr);
        }
        env_array.push(null());
        ...
        let ret = unsafe {
            minijail_run_pid_pipes_no_preload(
                ...
                env_array.as_ptr() as *const *mut c_char,
                &mut pid,
                null_mut(),
                null_mut(),
                null_mut(),
